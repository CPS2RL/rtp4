...
class ThriftAPI():
    ...
    def table_add(self, table_name, action_name, match_keys, action_params=[], prio=0):
        ...
        entries = self.table_dump(table_name)
        if entries:
          for entry in entries:
              if entry.match_key[0].exact.key == match_keys[0].exact.key:
                if entry.action_entry.action_name == action.name:

                     #print("Entry already exists for match_keys,skip")
                     return
                else:
                     print("action modified")
                     self.client.bm_mt_modify_entry(
                        0, table.name, entry.entry_handle, action.name, runtime_data
                     )
                     return
        #      else:
    ...
    def counter_read(self, counter_name, index):
        ...
        print("{}[{}]= {} packets".format(counter_name, index,value.packets))
