...
control MyIngress(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata) {
    counter(20, CounterType.packets) flow_packet_counter;
    ...
    table set_priority_h{
        key = {
            hdr.udp_deadline.flow_id:exact;
        }
        actions = {
            set_priority_1;
            set_priority_2;
    ...
    apply {
        ...
        if (hdr.ipv4.isValid()){
            ...
            register_timestamp.write(hdr.udp_deadline.flow_id-1,standard_metadata.ingress_global_timestamp);
            deadline.write(hdr.udp_deadline.flow_id-1,hdr.udp_deadline.deadline);
            weight.write(hdr.udp_deadline.flow_id-1,hdr.udp_deadline.weight);
            flow.write(hdr.udp_deadline.flow_id-1,hdr.udp_deadline.flow_id);
            packet_length.write(hdr.udp_deadline.flow_id,hdr.ipv4.totalLen);
            flow_packet_counter.count(hdr.udp_deadline.flow_id);
...
control MyEgress(inout headers hdr,
   ...
   apply {
        ...
        finish_time.write(hdr.udp_deadline.flow_id-1,standard_metadata.egress_global_timestamp);
